...
> 在当前项目中，`RunnableWithMessageHistory` 用于解释 LangChain 的消息历史注入机制；实际落地时，会话历史已经持久化到数据库 `messages` 表中，并通过 `MessageService` / `DbChatHistory` 转成 LangChain `BaseMessage[]`。因此生产链路不依赖进程内 `InMemoryChatMessageHistory`。
...
⚠️ **安全提醒**：`safePath()` 的路径沙箱校验必须放在工具内部，不能依赖模型“自觉遵守”。生产环境里，写操作还应接入审计日志，并进一步限制可写目录范围。
    
**写入类工具的生产建议**：`write_file` 这种通用文件写入工具在教学中便于演示，但生产环境中必须收敛到明确业务目录，不能让模型决定任意文件路径。当前项目更推荐通过业务服务写入 artifact、message 或数据库记录，而不是把文件系统写入暴露为通用能力。
</aside>
...
*   **向量存储**：当前项目使用 PostgreSQL + pgvector 持久化保存向量，服务重启后数据不会丢失。
...
LangChain community 也提供 `HuggingFaceTransformersEmbeddings` 封装，当前路径为 `@langchain/community/embeddings/huggingface_transformers`。但本项目没有使用这层封装，而是直接调用 `@xenova/transformers`，原因是当前依赖组合中 `@huggingface/transformers` / `onnxruntime-node` 存在版本兼容风险。

</aside>

当前项目实际使用 `@xenova/transformers` 直接调用 pipeline：
...
当前项目不单独维护 `VectorStoreService`。文档向量化由 `ChunkService` 负责：先用 `RecursiveCharacterTextSplitter` 切分文本，再用 `EmbeddingService.embedTexts()` 生成 384 维向量，最后写入 `document_chunks.embedding`。查询时由 `SearchService` 使用 pgvector 的 `<=>` 距离运算进行相似度排序。
...
- PostgreSQL + pgvector：持久化向量存储与相似度检索
如果只做 LangChain 官方内存向量库演示，才需要 `@langchain/community`、`@huggingface/transformers`、`@langchain/classic`。LangChain.js v1 中 `MemoryVectorStore` 应从 `@langchain/classic/vectorstores/memory` 引入，但当前项目不采用它作为主链路，因为内存向量库进程重启即丢失，无法支撑真实文档检索。
</aside>

当前项目的向量链路对应文件：

- `services/chat/src/document/chunk.service.ts` — 文档切分与向量化
- `services/chat/src/document/embedding.service.ts` — 嵌入模型调用
- `services/chat/src/document/search.service.ts` — 相似度检索
- `services/chat/prisma/schema.prisma` — `document_chunks` 表定义
...
⚠️ **关于教学验收接口**：当前项目不暴露独立的 `/api/embedding/store` 与 `/api/embedding/search` demo 接口。文档向量化属于文档处理流程，语义检索由聊天接口内部调用 `SearchService.similaritySearch()` 完成。下面的 `/api/embedding/*` 仅作为教学验收接口；按当前项目落地时，应接入已有文档上传、切分、向量化和会话检索流程。
